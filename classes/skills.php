...
class skills{
    public static function deleteSkillsForJob(string $jobID){
        $pdo = Database::connect();
        $query = "DELETE FROM JobSkills WHERE JobID = :jobid";
    ...
    public static function insertSoftSkillsJob(array $softSkills, string $jobID){
        $pdo = Database::connect();
        $acceptArray = array();
        $skillsStr = "";
        ...
        for($x=0; $x<count($skillIDs); $x++){
            $query = "INSERT INTO JobSkills (JobID, SkillID) VALUES (".$jobID.", ".$skillIDs[$x][0].");";
            $statement = $pdo->prepare($query);
            $statement->execute();
        }
    ...
    public static function insertSkillsForJob(array $languageYears, string $jobID){
        ...
        for($x=0; $x<count($basicSkills); $x++){
            ...
            if(!$languageYears[$x] == 0){
                ...
                $query3 = "INSERT INTO JobSkills VALUES (:jobid, :skillid)";
